...
class Base():
    ...
    def get(self,url):
        result = None
        reponse = requests.get(url,headers=self.get_headers)
        try:
            result = reponse.json()
    ...
    def post(self,url,data):
        """
        在原来post方法基础上，新增日志以及直接返回post格式
        :return：返回json格式
        """
        result = None
        reponse = requests.post(url,json=data,headers=self.get_headers())
        try:
            result = reponse.json() #获取响应体中的json数据
    ...
    def get_headers(self):
        """
        处理请求头
        :return: 返回的是字典格式的请求头，多是包含了Content-Type,X-Litemall-Admin-Token
        """
    ...
    def login(self):
        """
        通过调用登录接口，获取token值，将其进行缓存，其他接口使用时直接从缓存中获取数，若没有取到，再调用登录，再将token值放入缓存中
        :return:
        """
